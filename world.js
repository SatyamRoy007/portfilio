...
(() => {
  ...
  function render(){if(dirty)drawLandscape();ctx.imageSmoothingEnabled=false;ctx.drawImage(landscape,0,0);drawWood(ctx);[...workers].sort((a,b)=>a.y-b.y).forEach((w,i)=>villager(ctx,w,i));if(state.stage===2){const s=sites();villager(ctx,{x:s.newhut[0]-.025+Math.sin(elapsed*.13)*.01,y:s.newhut[1]+.012,home:s.newhut,color:'#688b80',role:'builder',wait:1},4);}drawAnimals(ctx);atmosphere(ctx);}
  ...
  function soundEnabled(enabled){
    ...
    if(!state.sound){audio?.context.suspend().catch(()=>{});announce();return;}
    try{
      if(!audio){const Audio=window.AudioContext||window.webkitAudioContext;if(!Audio){state.sound=false;announce();return;}const context=new Audio();const gain=context.createGain();gain.gain.value=.022;gain.connect(context.destination);const filter=context.createBiquadFilter();filter.type='lowpass';filter.frequency.value=650;filter.connect(gain);const buffer=context.createBuffer(1,context.sampleRate*2,context.sampleRate),data=buffer.getChannelData(0);let seed=5027,value=0;for(let i=0;i<data.length;i++){seed=(Math.imul(seed,1664525)+1013904223)>>>0;value=(value+(seed/4294967296*2-1)*.025)/1.005;data[i]=value;}const source=context.createBufferSource();source.buffer=buffer;source.loop=true;source.connect(filter);source.start();audio={context,filter,gain};}
      if(!document.hidden&&!state.paused)audio.context.resume().catch(()=>{state.sound=false;announce();});
      updateAudio();
    }catch(_){state.sound=false;}
    announce();
  }
  function updateAudio(){if(!audio)return;audio.filter.frequency.setTargetAtTime(state.weather==='rain'?1200:state.night?350:650,audio.context.currentTime,.5);audio.gain.gain.setTargetAtTime(state.weather==='rain'?.045:.022,audio.context.currentTime,.5);if(state.paused||document.hidden||!state.sound)audio.context.suspend().catch(()=>{});else audio.context.resume().catch(()=>{});}
  function loop(now){
    ...
    if(dayClock>=90){dayClock-=90;state.day=Math.min(99999,state.day+1);persist();announce();}
    if(saveClock>8){saveClock=0;persist();}
    render();
  }
  window.villageWorld={
    setSeason(value){if(!seasons.includes(value)||value===state.season)return;state.season=value;changed();},
    setWeather(value){if(!weathers.includes(value)||value===state.weather)return;state.weather=value;updateAudio();changed();},
    setNight(value){state.night=Boolean(value);updateAudio();changed();},
    setPaused(value){state.paused=Boolean(value);lastFrame=performance.now();updateAudio();changed(false);},
    setEffects(value){state.effects=Boolean(value);changed(false);},
  ...
  document.addEventListener('visibilitychange',()=>{lastFrame=performance.now();updateAudio();if(document.hidden)persist();else render();});
  window.addEventListener('pagehide',()=>{persist();audio?.context.suspend().catch(()=>{});});
